...
Writing this suite surfaced two real, previously-undiscovered bugs, both now fixed:
...
2. **Suspicious negative duration in `CheckOut` server logs.** During `TestAttendance_CheckInCheckOutCycle`, the backend logged `Duration: -2.00 hours` for a check-in immediately followed by a check-out - both happening within the same test, milliseconds apart. This points to a timezone round-trip mismatch between the `time.Time` written to Postgres and the value read back for the duration calculation in `attendance.go`. Not yet fixed - flagged here for follow-up.
